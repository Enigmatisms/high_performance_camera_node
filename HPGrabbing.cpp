...
主要内容：
    ...
    3.目前帧数最大值：88fps
    4.如果在等待循环中加入printf,帧数为60fps，这不行
*/
...
void g_pub_image(){
    ...
    //printf("Frame info(%d, %d), empty[%d].\n",
    //    g_frame.cols, g_frame.rows, g_frame.empty());
    g_pub.publish(msg);
}
...
bool g_isWritable(int pos){       
    ...
    #endif
    while(g_buf[pos].w_tag != 1){                          //出现问题的等待循环在此处
        printf("g_buf[%d].w_tag = %d.\n", pos, g_buf[pos].w_tag);
    }                //无超时保护,死循环
...
void g_decoding(int pos){
    ...
    cv::Mat yuyv_img(1080, 1440, CV_8UC2);
    memcpy(yuyv_img.data, g_buf[pos].frame.pBufAddr, FRAME_SIZE*sizeof(uchar));
    cv::cvtColor(yuyv_img, g_frame, cv::COLOR_YUV2RGB_YUYV);
    g_pub_image();
    #ifdef DEBUG
...
void* g_writeWorkThread(void* pUser){
    ...
    while(1){
        ...
        if(next_pos == -1){
            #ifdef DEBUG
                printf("No data[0x%x].\n", error);
                next_pos = pos;            //如果写入失败，就尝试一直在此位置写入，直到成功为止,目前没有出现问题)
            #endif
        }
